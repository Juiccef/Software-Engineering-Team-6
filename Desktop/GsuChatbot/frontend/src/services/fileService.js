...
/**
 * Upload a transcript for schedule planning pipeline
 * @param {File} file - The transcript file to upload
 * @param {string} sessionId - Current chat session ID
 * @param {function} onProgress - Callback for upload progress (optional)
 * @returns {Promise<Object>} Upload result
 */
export const uploadTranscript = async (file, sessionId, onProgress = null) => {
    try {
        // Validate file
        if (!file) {
            throw new Error('No file provided');
        ...
        if (!sessionId) {
            throw new Error('Session ID is required for transcript upload');
        }
        ...
        // Only allow PDFs for transcripts
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.open('POST', `${API_BASE_URL}/upload-transcript`);
            xhr.send(formData);
        });
