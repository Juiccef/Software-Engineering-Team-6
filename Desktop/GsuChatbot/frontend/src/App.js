...
function App() {
  ...
  const [chatSettings, setChatSettings] = useState(() => {
    ...
    return null; // Will use defaults from backend
  });
  ...
  const startNewChat = () => {
    ...
    setScreen("home"); // Go to home screen to show quick suggestions
    // Pipeline state is stored in messages, so resetting messages resets pipeline state
  };
  ...
  const handleSendMessage = async (message) => {
    ...
    let fallbackResponse = '';
      ...
      const botResponse = {
        ...
        inPipeline: data.inPipeline || false,
        pipelineState: data.pipelineState || null,
        pipelineExited: data.pipelineExited || false,
  ...
  const handleQuickAction = (actionId) => {
    const actionMessages = {
      ...
      schedule: "I need help building my class schedule for next semester",
  ...
  return (
    <div style={{
      ...
      <Layout
        ...
        showChatBox={screen === "chat" || screen === "home"}
        mode={mode}
        isTyping={chatSettings?.typingIndicator !== false ? isTyping : false}
